...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      justifyContent: 'center', background: 'var(--bg-dark, #0a0f1e)'
    }}>
      <div className="glass-panel" style={{ padding: '2.5rem', width: '100%', maxWidth: '380px' }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          <div className="brand-logo-icon" style={{ margin: '0 auto 0.75rem' }}>LT</div>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          {error && (
            <p style={{ color: 'var(--risk-high)', fontSize: '0.82rem', margin: 0 }}>
              {error}
            </p>
          )}
        ...
        <p style={{ color: 'var(--text-dark)', fontSize: '0.75rem', textAlign: 'center', marginTop: '1.5rem' }}>
...
export default function App() {
  ...
  const fetchCases = async () => {
    ...
    } catch (e) { console.error('fetchCases:', e.message); }
  };
  ...
  const fetchAlertLogs = async () => {
    ...
    } catch (e) { console.error('fetchAlertLogs:', e.message); }
  };
  ...
  const handleSearch = async (addr, chainInput) => {
    ...
    try {
      ...
      const chain = targetChain === 'ETH' ? 'ethereum'
                  : targetChain === 'BTC' ? 'bitcoin'
                  : null;
      ...
      const response = await apiFetch(`/api/trace/address/${targetChain}/${targetAddress}`);
      if (!response.ok) {
  ...
  const triggerSimulation = async () => {
    try {
      const response = await apiFetch('/api/monitor/simulate', { method: 'POST' });
      const data = await response.json();
      if (data.triggered) { fetchAlertLogs(); fetchStats(); }
  ...
  const saveCurrentCase = async () => {
    ...
    const name = prompt('Enter investigation case name:', `Case - ${tracedData.details?.name || tracedData.address}`);
    if (!name) return;
    try {
    ...
    <div className="app-container">
      <header className="app-header">
        ...
        <nav className="nav-links">
          <button className={`nav-btn ${activeTab === 'dashboard'   ? 'active' : ''}`} onClick={() => setActiveTab('dashboard')}>📊 Command Dashboard</button>
          <button className={`nav-btn ${activeTab === 'explorer'    ? 'active' : ''}`} onClick={() => setActiveTab('explorer')}>🕸️ Trace Graph Explorer</button>
          <button className={`nav-btn ${activeTab === 'monitoring'  ? 'active' : ''}`} onClick={() => setActiveTab('monitoring')}>🔔 Realtime Monitoring</button>
          <button className={`nav-btn ${activeTab === 'compliance'  ? 'active' : ''}`} onClick={() => setActiveTab('compliance')}>📋 Compliance Reports</button>
          <button className={`nav-btn ${activeTab === 'antigravity' ? 'active' : ''}`} onClick={() => setActiveTab('antigravity')}>⚛️ Gravity Lab & Reports</button>
        </nav>
        ...
        <div className="user-badge" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <button
            ...
            style={{ background: 'transparent', border: '1px solid var(--border-light)', color: 'var(--text-muted)', borderRadius: '4px', padding: '0.2rem 0.6rem', cursor: 'pointer', fontSize: '0.75rem' }}
          >
            Logout
          </button>
        </div>
      ...
      <div className="dashboard-grid">
        ...
        <aside className="glass-panel search-sidebar">
          <div className="sidebar-section">
            ...
            <div className="form-group">
              ...
              <select className="select-input" value={searchChain} onChange={e => setSearchChain(e.target.value)}>
            ...
            {errorMessage && (
              <p style={{ color: 'var(--risk-high)', fontSize: '0.8rem' }}>{errorMessage}</p>
            )}
            ...
            <div className="history-list">
              {history.length === 0 ? (
                ...
                history.map((item, idx) => (
                  <div key={idx} className="history-card" onClick={() => handleSuggestionClick(item.address, item.chain)}>
          ...
          <div className="sidebar-section">
            ...
            <div className="history-list" style={{ maxHeight: '150px' }}>
              {cases.map((c, idx) => (
                <div key={idx} className="history-card" style={{ borderLeft: '3px solid var(--color-primary)' }}
        ...
        <main style={{ minWidth: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              ...
              <div className="glass-panel" style={{ padding: '1.5rem', background: 'linear-gradient(135deg, rgba(13,20,38,0.9) 0%, rgba(20,10,50,0.4) 100%)', border: '1px solid var(--border-accent)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
                  ...
                  <button className="btn-primary" onClick={triggerSimulation}>🚀 Simulate Block Event</button>
                </div>
              </div>
              ...
              <div className="glass-panel" style={{ padding: '1.5rem' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1rem' }}>
                  <div className="history-card" style={{ border: '1px solid rgba(239,68,68,0.2)', padding: '1rem' }}
                    onClick={() => handleSuggestionClick('1LbcPeel5s9zARansom993vX78cDf', 'BTC')}>
                    <h4 style={{ color: 'var(--risk-high)', fontWeight: 700 }}>🔴 BTC LockBit Ransomware</h4>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '0.25rem 0' }}>Showcases a dynamic peeling chain where extorted Bitcoin is laundered down.</p>
                    <code style={{ fontSize: '0.75rem', color: '#fff' }}>1LbcPeel5s9zARansom993vX78cDf</code>
                  </div>
                  <div className="history-card" style={{ border: '1px solid rgba(245,158,11,0.2)', padding: '1rem' }}
                    onClick={() => handleSuggestionClick('0x71c20e241775e5332f143715df332f143789a71b', 'ETH')}>
                    <h4 style={{ color: 'var(--risk-medium)', fontWeight: 700 }}>🟡 ETH Tornado Cash Router</h4>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '0.25rem 0' }}>Traces funds routed through an automated sanctioned privacy pool.</p>
                    <code style={{ fontSize: '0.75rem', color: '#fff' }}>0x71c20e241775e5332...f143789a71b</code>
                  </div>
                  <div className="history-card" style={{ border: '1px solid rgba(0,242,254,0.2)', padding: '1rem' }}
                    onClick={() => handleSuggestionClick('HN7c5P28vPj3p83Vz18djs83hV9as8a8d11c8eD', 'SOL')}>
                    <h4 style={{ color: 'var(--color-primary)', fontWeight: 700 }}>🔵 SOL Mango Markets Exploiter</h4>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', margin: '0.25rem 0' }}>Solana network asset routing layout following a large-scale DeFi exploit.</p>
                    <code style={{ fontSize: '0.75rem', color: '#fff' }}>HN7c5P28vPj3p83Vz18djs83hV9as8a8d11c8eD</code>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-panel" style={{ padding: '1.5rem' }}>
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table className="logs-table">
                    <thead>
                      <tr>
                        <th>Risk Level</th><th>Network</th><th>Address</th><th>Alert Trigger Message</th><th>Timestamp</th>
                      </tr>
                    </thead>
                    <tbody>
                      {alertLogs.length === 0 ? (
                        ...
                        alertLogs.map(log => (
                          <tr key={log.id}>
                            <td><span className={`severity-tag ${log.severity}`}>{log.severity === 'critical' ? '🔴 Critical' : '🟡 Warning'}</span></td>
                            <td><strong>{log.chain}</strong></td>
                            <td>
                              <a href="#" className="addr-link" onClick={e => { e.preventDefault(); handleSuggestionClick(log.address, log.chain); }}>
              ...
              {tracedData ? (
                <>
                  ...
                  <div className="analysis-sidebar">
                    <RiskPanel riskData={tracedData} onNodeClick={addr => handleSearch(addr, tracedData.chain)} />
                  </div>
                </>
              ) : (
          ...
          {activeTab === 'monitoring'  && <MonitorPanel alertLogs={alertLogs} onAddressSelect={handleSuggestionClick} onSimulationTrigger={triggerSimulation} fetchAlertLogs={fetchAlertLogs} />}
          {activeTab === 'compliance'  && <ReportView   tracedData={tracedData} cases={cases} onLoadCase={handleSuggestionClick} />}
